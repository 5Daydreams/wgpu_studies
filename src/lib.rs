...
impl State {
    ...
    async fn new(window: &Window) -> Self {
        ...
        let config = wgpu::SurfaceConfiguration {
            ...
            width: size.width,
            height: size.height,
            present_mode: wgpu::PresentMode::Fifo, // Essentially V-sync
        ...
        Self {
            surface, // surface -> a "rect" on the window
            device,  // device -> a reference to a "virtual graphics card"
            queue,   // queue -> receives commands to be sent into a wavefront
            config,  // config -> settings to apply into a surface
            size,    // size -> the size in pixels of the surface to be used for rendering
            render_pipeline,
        }
    }
...
pub async fn run() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_arch = "wasm32")] {
            ...
            env_logger::init(); // Sacred line - this is required for GPU debugging
        }
    }
